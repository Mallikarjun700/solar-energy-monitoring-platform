...
class DashboardService
{
    public function getDashboard(string $tenantId): array
    {
        ...
        $latestTelemetry = $this->latestTelemetryForDevices();

        return [
            'kpis' => $this->getKpis($latestTelemetry, $startOfToday),
            'plants' => $this->getPlants($latestTelemetry),
            'devices' => $this->getDevices($latestTelemetry),
            'energyTrend' => $this->getEnergyTrend($startOfToday),
            'alerts' => $this->getAlerts($tenantId),
            'recentTelemetry' => $this->getRecentTelemetry(),
            'recentActivity' => $this->getRecentActivity($tenantId),
        ];
    }

    private function getKpis(Collection $latestTelemetry, Carbon $startOfToday): array
    {
    ...
    private function getPlants(Collection $latestTelemetry): array
    {
        ...
        return Plant::query()
            ->orderBy('id')
            ->get()
            ->map(function (Plant $plant) use ($devicesByPlant, $latestByDevice): array {
                ...
                return [
                    ...
                    'currentPowerKw' => $hasTelemetry
                        ? round($power, 2)
                        : null,
                    'performancePercent' => $capacity !== null && $capacity > 0 && $hasTelemetry
    ...
    private function getDevices(Collection $latestTelemetry): array
    {
        $latestByDevice = $latestTelemetry->keyBy('device_id');
    ...
    private function getEnergyTrend(Carbon $startOfToday): array
    {
        return Telemetry::query()
            ...
            ->get()
            ->map(fn ($row): array => [
                'timestamp' => Carbon::parse($row->bucket)->toISOString(),
    ...
    private function getRecentTelemetry(): array
    {
        return Telemetry::query()
    ...
    private function latestTelemetryForDevices(): Collection
    {
